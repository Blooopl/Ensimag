...
#
  rmoy_unif <- function(n_simulations, n_sample){
    replicate(n_simulations, mean(complete_moi))
    }

# moyennes empiriques d'échantillons U(0,1)  
  x_bar <-  rmoy_unif(100000, n_sample = change_moi)
```
...
cette valeur dans la simulation ?

**Solution.** Code à commenter et à compléter.

```{r}
# commenter le code
mean( abs(change_moi) < 0.1 ) > 0.95
```


```{r}
# Tester d'autres valeurs n = 10, 20, 30, 40, etc
  x_bar <-  rmoy_unif(10000, n_sample = change_moi)
...
de décimale près peut-on prévoir que l'estimation est précise ?
...
Calculer la variance de $Z_n$. Converge-t-elle vers 0 ? On suppose
...
ry <- function(nsimu, nsample)
{
  ...
  # commenter la valeur renvoyée
  return(replicate(nsimu, ygen()))
}

# commenter
z <- (ry(10000, 100) - pi)*completer

# commenter 
# help(dnorm)
hist(z, prob = TRUE, col = "lightblue")
curve(densite_connue(x), add=TRUE, col="blue", lwd=2)
```
...
```{r}
 ...
 # help(qqnorm)
 qqnorm(trouve_ce_que_cest_ici, cex = .4, col = "red4")
...
```{r, eval=TRUE}
 ...
 # definition de Z
 z = apply(X, MARGIN = 1, mean) - m)*complete_moi
