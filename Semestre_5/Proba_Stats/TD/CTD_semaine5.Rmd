...
Calculer la fonction de vraisemblance du paramètre $\theta$. 
Montrer que l'estimateur du maximum de vraisemblance de $\theta$ est la moyenne empirique.
...
```{r}
c(a = quelque_chose_ici, b = quelque_chose_la)
```
...
On répète 10000 fois l'échantillonnage de la population effectué dans la question 2. 
Vérifier que la vraie valeur, $\theta = 2.4$, se trouve dans l'IC pour environ 95% des echantillons ainsi créés.
...
intervalle_IC <- function()
{
  ...
  booleen_theta_dans_IC_pour_l_echantillon_x.rep 
}
...
Que représente $1/\theta$ ? Proposer une estimation du paramètre $\theta$ par la méthode des moments, puis par maximum de vraisemblance. 
...
Un passionné de football affirme qu'en moyenne dans un match de championnat de France, le premier but est observé avant la 25eme minute.
...
**Solution.**  
On peut calculer la $p$-valeur à partir de la loi Gamma et de l'observation $s_n$. 
On notera qu'une fonction pivot n'est pas nécessaire. Expliquer les choix effectués.
