...
class ThemeSwitcher {
    ...
    switchTheme() {
		...
		document.querySelector("body")?.setAttribute("class", this.#scheme);
		document.cookie = `mode=${this.#scheme}; path=/` 
    }
    
    changeIcon() {
		this.#target.innerHTML = (this.#scheme == "light" ? moonHTML : sunHTML);
    }
}

window.addEventListener('load', () => new ThemeSwitcher(document.getElementById("theme-switcher")))
