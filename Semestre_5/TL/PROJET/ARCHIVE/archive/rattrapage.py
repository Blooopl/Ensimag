...
"""On reprend le fichier calc.py en rattrapant de la façon suivante :
Quand on peut, on supprime les caractères "qui nous embête".

On est dans le cas d'une grammaire LL1 donc le principale problème du rattrapage est : 
De savoir quoi faire quand on a déjà lu un caractère a qui impliquait un caractère b plus tard et que ce b n'existe pas.

Il faut alors faire au cas par cas :
    -Pour les parenthèses ouvertes mais pas fermés on fait comme si y'en avait pas eu
    -Pour les puissances , on pose que l'exposant vaut 1
    -Pour les Sommes et additions, on pose que la deuxième opérande vaut 0
"""
def parse_input(L=[]):
    match get_current():
        case V_T.END:
